...
public class Solution
{
    ...
    public int FindTargetSumWays(int[] nums, int target)
    {
        ...
        if (neg < 0 || (nums.Sum() - target) % 2 != 0)
            return 0;
        //dp[i][j]表示前i个物品，背包容量为j时的最大价值,dp[0][j] = 1, dp[i][0] = 1(只有一种情况，不放任何东西)
        ...
        for (int i = 1; i <= nums.Length; i++)
        {
            for (int j = 0; j <= neg; j++)
            {
                //第i个物品的重量为nums[i-1],价值为1
                if (j - nums[i - 1] < 0)//这个物品放不下了
                    dp[i, j] = dp[i - 1, j];
                else//这个物品放的下，选择放或者不放,所有可能情况是二者的和
